...
This script creates a new folder with only the essential files needed for
the Monte Carlo trading application to function, removing clutter from
demo files, test files, and documentation.
"""
...
def create_summary_file(clean_project_name, copied_files, skipped_files):
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write("\n\nCreated by: Monte Carlo Project Cleaner\n")
...
def show_project_stats():
    ...
    print(f"  📁 Total files: {len(essential_python) + len(essential_docs) + 10}")  # +10 for algorithms and other essentials
    print(f"  🐍 Python files: {len(essential_python)}")
    print(f"  📖 Documentation: {len(essential_docs)}")
    print(f"  📉 Reduction: ~{((len(current_files) - (len(essential_python) + len(essential_docs) + 10)) / len(current_files) * 100):.1f}%")

if __name__ == "__main__":
    print("🧹 Monte Carlo Project Cleaner")
    print("=" * 35)
    ...
    project_name = input("Enter name for clean project directory [monte_carlo_clean]: ").strip()
    if not project_name:
        project_name = "monte_carlo_clean"

    # Confirm
    response = input(f"\nCreate clean project in '{project_name}'? (y/n): ").strip().lower()
